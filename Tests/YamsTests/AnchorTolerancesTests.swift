...
class AnchorTolerancesTests: XCTestCase {
    ...
    /// Any type that is Encodable and contains an `Anchor`value but with a coding key different from YamlAnchorProviding
    /// will not encode to a yaml anchor
    ...
    func testAnchorEncoding_undeclaredBehavior() throws {
        ...
        /// Any type that is Encodable and contains an `Anchor`value with the same coding key as YamlAnchorProviding
        /// will encode to a yaml anchor even though the type does not conform to YamlAnchorProviding
        /// This may be unexpected
    ...
    /// Any type that is Decodable and contains an `Anchor` value but with a coding key different from YamlAnchorProviding
    /// will not decode an anchor from the text representation.
    ...
    /// Any type that is Decodable and contains an `Anchor` value with the same coding key as YamlAnchorProviding
    /// will decode an anchor from the text representation even though the type does not conform to YamlAnchorCoding
    /// This may be unexpected
